...
export function InventoryList() {
  ...
  const [newItem, setNewItem] = useState<Omit<StockItem, "id" | "created_at" | "updated_at">>({
    product_name: "",
    product_id: "",
    quantity: 0,
    unit_price: 0,
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const response = await fetch("http://localhost:3333/api/stocks/test?page=1&limit=50")
      if (!response.ok) {
  ...
  const handleSaveEdit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3333/api/stocks/${editItem.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`
  ...
  const handleDeleteConfirmed = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3333/api/stocks/${itemToDelete}`, {
        method: "DELETE",
        headers: {
          "Authorization": `Bearer ${token}`
        }
              ...
              {filteredInventory.length === 0 ? (
                ...
                filteredInventory.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>{item.branch_id.slice(0, 8)}...</TableCell>
                    ...
                    <TableCell className="flex space-between content-center">
                      ...
                      >
                        ...
                        <span className="sr-only">Edit {item.product_name}</span>
                      </Button>
                      <Button
                        variant="ghost"
                        size="icon"
                      ...
                      >
                        ...
                        <span className="sr-only">Delete {item.product_name}</span>
                      </Button>
                    </TableCell>
                  </TableRow>
                ))
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>Make changes to the stock item here. Click save when you're done.</DialogDescription>
          </DialogHeader>
          <form
            onSubmit={(e) => {
              e.preventDefault();
            ...
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input
                  ...
                  onChange={(e) => setEditItem(editItem ? { ...editItem, product_name: e.target.value } : null)}
                  required
                />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="item-product-id" className="text-right">
                  Product ID
                </Label>
                <Input
                  id="item-product-id"
                  ...
                  className="col-span-3"
                  onChange={(e) => setEditItem(editItem ? { ...editItem, product_id: e.target.value } : null)}
                  required
                />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input
                  ...
                  className="col-span-3"
                  onChange={(e) => setEditItem(editItem ? { ...editItem, supplier: e.target.value } : null)}
                  required
                />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input
                  ...
                  className="col-span-3"
                  onChange={(e) =>
                    setEditItem(editItem ? { ...editItem, unit_price: Number.parseFloat(e.target.value) } : null)
                  }
                  required
                />
              </div>
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="item-branch" className="text-right">
                  Branch ID
                </Label>
                <Input
                  id="item-branch"
                  ...
                  className="col-span-3"
                  onChange={(e) => setEditItem(editItem ? { ...editItem, branch_id: e.target.value } : null)}
                  required
                />
              </div>
            </div>
      ...
      <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>Add a new stock item. Click save when you're done.</DialogDescription>
          </DialogHeader>
          <form
            onSubmit={(e) => {
              e.preventDefault();
          ...
          >
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="new-item-product-id" className="text-right">
                  Product ID
                </Label>
                <Input
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="new-item-supplier" className="text-right">
                  Supplier
                </Label>
                <Input
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="new-item-branch" className="text-right">
                  Branch ID
                </Label>
                <Input
                  id="new-item-branch"
                  ...
                  className="col-span-3"
                  onChange={(e) => setNewItem({ ...newItem, branch_id: e.target.value })}
                  required
                />
              </div>
            </div>
