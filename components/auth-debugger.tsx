...
export function AuthDebugger() {
  ...
  const runDebugTests = async () => {
    ...
    try {
      ...
      testResults.clientCreation = { success: false, error: clientError.message }
      testResults.authentication = { success: false, error: 'Could not create client' }
    }

    // Test 4: Check if backend is reachable
    try {
      const response = await fetch('http://localhost:3333/api/stocks/test')
    ...
    <div className="space-y-4 p-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Debug authentication issues step by step</CardDescription>
        </CardHeader>
        <CardContent>
