...
export async function fetchDashboardStats(): Promise<DashboardStats> {
  try {
    if (API_CONFIG.USE_BACKEND_API) {
      ...
      return await apiRequest<DashboardStats>(API_CONFIG.ENDPOINTS.DASHBOARD_STATS)
    }
...
export async function fetchSalesTotal(period?: string): Promise<SalesTotalResponse | undefined> {
  try {
    // working now 
    if (API_CONFIG.USE_BACKEND_API) {
...
export async function fetchChartData(): Promise<ChartData> {
  try {
    if (API_CONFIG.USE_BACKEND_API) {
      // Use actual backend API
      return await apiRequest<ChartData>(API_CONFIG.ENDPOINTS.DASHBOARD_CHARTS)
    }
